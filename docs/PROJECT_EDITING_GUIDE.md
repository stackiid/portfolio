...
`cfSubmitBtn` starts disabled and is re-evaluated by `updateSubmitState()` on every `input` event
and on tab switch. It enables only once all required fields for the active mode pass validation,
and disables again immediately if any field becomes invalid.
