...
#### Liquid Scroll-Progress Header Fill

- `.glass-header` now includes a `.header-progress-fill` layer that grows left-to-right in sync
  with page scroll position — 0% width at the top of the page, 100% at the bottom.
- New `initHeaderScrollProgress()` in `scripts/app.js` computes
  `progress = scrollY / (scrollHeight - innerHeight)` on a rAF-throttled, passive `scroll`
  listener (with a matching `resize` listener) and writes the result to `style.width` on the
  fill element. Only `width` on a small absolutely-positioned layer is touched per frame — no
  layout thrash, no jank on scroll in either direction.
- `.glass-header` gained `overflow: hidden` so the fill is clipped to the header's existing
  rounded corners, and `.header-content` gained `position: relative; z-index: 1` so the logo,
  nav links, and theme toggle always render above the fill (contrast and readability preserved).
- The fill uses the existing palette: `linear-gradient(180deg, rgba(var(--accent-rgb), 0.22),
  rgba(var(--accent-rgb), 0.08))` — no new colours introduced.

#### Animated Wave Effect Inside Header Progress Fill

- Two `.header-progress-wave` elements (low-opacity gold blobs built from `--accent-rgb`) sit at
  the fill's leading edge and continuously rotate via `@keyframes header-wave-spin` /
  `header-wave-spin-reverse` (9s and 13s linear loops, opposite directions).
- Because the rotation is a pure CSS animation independent of the scroll listener, the liquid
  surface keeps moving even when scrolling stops, as required.
- Wrapped in `@media (prefers-reduced-motion: reduce)` to disable the spin animation for users
  who have requested reduced motion.
- Added to the header markup on `index.html`, `pages/about.html`, and `pages/services.html`.

#### Enhanced Contact Form Validation

- New `validateFullName()` enforces strict "First Last" input: at least two words, each
  alphabetic with optional internal apostrophes/hyphens (`O'Brien`, `Anne-Marie`), no digits or
  other special characters. Multiple spaces are normalised and leading/trailing spaces trimmed
  on blur. Invalid input shows *"Please enter your first and last name."*
- New `isFormValid()` / `updateSubmitState()` gate `#cfSubmitBtn`: the button is disabled by
  default and only enables once every required field for the active mode (Full Name + Message +
  Email + Subject for Mail, or Full Name + Message + WhatsApp Number for WhatsApp) passes
  validation. It re-disables immediately if any field becomes invalid again.

### Changed

- Updated all GitHub username references from `stackubaii` to `stackiid` across `index.html`
  (canonical link, Open Graph URL, JSON-LD `url`/`sameAs`, footer/social link) and `README.md`
  (live preview link, deployment notes).
- `scripts/contact-form-validation.js` was consolidated from two duplicated, independently-bound
  IIFEs into a single module. Both blocks previously attached their own `submit`/`input`
  listeners to the same `#contactForm`, causing duplicate WhatsApp window opens and duplicate
  Formspree POST requests on every send.
- Real-time field validation, error messaging, and the submit button's enabled/disabled state
  are now driven by a single shared set of validator functions
  (`validateFullName`, `validateEmail`, `validateWhatsApp`, `validateSubject`, `validateMessage`).

### Fixed

- Form submission is now blocked until the Full Name field contains a valid first and last name.
- Removed the double-submission bug caused by the duplicated validation script (see *Changed*).
- Resolved inconsistent error-display behaviour between the two previously-duplicated
  validation blocks (only one wired its error spans to `#cf-name-error`, `#cf-subject-error`,
  and `#cf-message-error`; the other left them empty).
